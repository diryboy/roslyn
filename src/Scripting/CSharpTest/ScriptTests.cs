...
");
        ...
        public async Task StaticDelegate0()
        {
            ...
            var state1 = await state0.ContinueWithAsync("Func<int, int, int> adder = Add;");
        ...
        {
            var state0 = await CSharpScript.RunAsync("class Id<T> { static T Core(T t) => t; public static Func<T, T> Get => Core; }");
        ...
        {
            var state0 = await CSharpScript.RunAsync("class Id { static T Core<T>(T t) => t; public static Func<T, T> Get<T>() => Core; }");
